...
class Coordinate {
  ...
  void paint(
    ...
    for (int i = 1; i < xScaleCount; i++) {
      ...
      textPainter.paint(
          canvas, Offset(i * step - textPainter.size.width / 2, 5));
      canvas.drawLine(Offset(i * step, 0), Offset(i * step, 5), axisPaint);
      canvas.drawLine(
          Offset(i * step, 0), Offset(i * step, -size.height), gridAxisPaint);
    }
    ...
    for (int i = 1; i < yScaleCount; i++) {
      ...
      textPainter.paint(
          canvas,
          Offset(-textPainter.size.width - 5,
              -(i * stepY + textPainter.size.height / 2)));
      canvas.drawLine(Offset(0, -i * stepY), Offset(5, -i * stepY), axisPaint);

      canvas.drawLine(
          Offset(0, -i * stepY), Offset(size.width, -i * stepY), gridAxisPaint);
    }
